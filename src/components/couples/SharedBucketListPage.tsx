// src/components/couples/SharedBucketListPage.tsx
...
const STATUS_META: Record<BucketStatus, { label: string; headGradient: string; countColor: string }> = {
  not_started: { label: 'Not started', headGradient: 'from-[#e88584] to-[#8e4f63]', countColor: 'text-white/85' },
  planning:    { label: 'Planning',    headGradient: 'from-[#0068aa] to-[#004d7f]', countColor: 'text-white/85' },
  in_progress: { label: 'In progress', headGradient: 'from-[#FFA600] to-[#B36B00]', countColor: 'text-white/85' },
  booked:      { label: 'Booked',      headGradient: 'from-[#B1E006] to-[#6C8300]', countColor: 'text-white/85' },
  completed:   { label: 'Completed',   headGradient: 'from-[#01B1AF] to-[#018a88]', countColor: 'text-white/90' },
};
...
const CATEGORY_OPTIONS = [
  'Travel','Adventure','Personal Growth','Career','Health & Fitness',
  'Creative','Social','Learning','Financial','Family','Other',
];
...
/** Same logic as CoupleActivityTracker: use inviter as the shared owner */
...
/* ---------------- Component ---------------- */
export default function SharedBucketListPage() {
  const { hasAccess, currentPlan } = useSubscription();
  const isPremium = hasAccess('couples-bucket-list');

  if (!isPremium) {
    return (
      <FeatureAccessGuard featureId="couples-bucket-list" currentPlan={currentPlan}>
        <SharedBucketListPageContent />
      </FeatureAccessGuard>
    );
  }
  return <SharedBucketListPageContent />;
}

function SharedBucketListPageContent() {
  ...
  const loadItems = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select('id, owner_user_id, created_by_user_id, title, status, category, priority, target_date, location, budget_estimate, emotional_meaning, description, completed_at, color_index, created_at, updated_at')
  ...
  const loadSettings = useCallback(async () => {
    ...
    else setSettings({ owner_user_id: ownerId, share_enabled: false, updated_at: new Date().toISOString() });
  }, [ownerId]);

  useEffect(() => { void loadItems(); void loadSettings(); }, [loadItems, loadSettings]);

  // keep invitee view empty when sharing is off (parity with tracker)
  useEffect(() => {
    if (isInvitee && settings && !settings.share_enabled) setItems([]);
  }, [isInvitee, settings]);
  ...
  const byStatus = useMemo(() => {
    const g: Record<BucketStatus, BucketItem[]> = {
      not_started: [], planning: [], in_progress: [], booked: [], completed: [],
    };
  ...
  const toggleSharing = useCallback(async () => {
    if (!ownerId || isInvitee) return; // invitee can’t switch
    const next = !(settings?.share_enabled ?? false);
    const { data } = await supabase
      .from('couple_bucket_settings')
      .upsert(
        { owner_user_id: ownerId, share_enabled: next, updated_at: new Date().toISOString() },
        { onConflict: 'owner_user_id' }
      )
      .select('owner_user_id, share_enabled, updated_at')
      .single();
    if (data) setSettings(data as SettingsRow);
  }, [ownerId, isInvitee, settings?.share_enabled]);

  const addItem = useCallback(async () => {
    if (!ownerId) return;
    const payload = {
      owner_user_id: ownerId,
      created_by_user_id: userData?.id ?? ownerId, // stamp the real creator
      title: 'New idea',
      status: 'not_started' as BucketStatus,
      category: null,
      priority: 1,
  ...
  const saveEditor = useCallback(async () => {
    ...
    const nextCompletedAt = editor.status === 'completed' ? (editor.completed_at || todayISO()) : null;
    ...
    const { data, error } = await supabase
      ...
      .single();
    setSaving(false);
    if (error || !data) { alert('Could not save. Please try again.'); return; }
  ...
  const quickComplete = (row: BucketItem) => async () => {
    ...
    const { error, data } = await supabase
      ...
      .select('id, owner_user_id, created_by_user_id, title, status, category, priority, target_date, location, budget_estimate, emotional_meaning, description, completed_at, color_index, created_at, updated_at')
      .single();
    if (error) setItems((prev) => prev.map((r) => (r.id === row.id ? row : r)));
    if (data) setItems((prev) => prev.map((r) => (r.id === row.id ? (data as BucketItem) : r)));
  };
  ...
  const deleteItem = async () => {
    ...
    setSaving(true);
    const { error } = await supabase.from('couple_bucket_items').delete().eq('id', editor.id);
    setSaving(false);
  ...
  const onDropToStatus = (toStatus: BucketStatus) => async (e: React.DragEvent) => {
    ...
    const optimistic = { ...row, status: toStatus, completed_at: toStatus === 'completed' ? todayISO() : null };
    setItems((prev) => prev.map((r) => (r.id === id ? optimistic : r)));

    const { error, data } = await supabase
      .from('couple_bucket_items')
      .update({ status: toStatus, completed_at: toStatus === 'completed' ? todayISO() : null } as any)
      .eq('id', id)
      .select('id, owner_user_id, created_by_user_id, title, status, category, priority, target_date, location, budget_estimate, emotional_meaning, description, completed_at, color_index, created_at, updated_at')
      .single();

    if (error) setItems((prev) => prev.map((r) => (r.id === id ? row : r)));
    else if (data) setItems((prev) => prev.map((r) => (r.id === id ? (data as BucketItem) : r)));
  };
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-10">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <p className="text-red-800 font-medium">Unable to load bucket list</p>
  ...
  // Invitee cannot view until sharing is ON (mirror tracker UX)
  if (isInvitee && settings && !settings.share_enabled) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-10">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
          <h2 className="text-lg font-semibold text-blue-900">Sharing is off</h2>
          <p className="text-sm text-blue-800 mt-2">
            Your partner hasn’t enabled sharing for the Shared Bucket List yet.
            Once they toggle it on in <span className="font-medium">Settings → Partner</span>, you’ll see items here.
          </p>
        </div>
      </div>
    );
    ...
    <div className="w-full">
      ...
      <div className="relative rounded-xl overflow-hidden bg-gradient-to-b from-[#01B1AF] to-[#018a88] p-8 mb-12">
        <div className="relative z-10 flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">Shared Bucket List</h1>
            <p className="text-base text-white/80">Dreams & experiences to create together</p>
          </div>
        ...
        <div className="flex items-center gap-3">
            <button
              ...
              } ${isInvitee ? 'opacity-60 cursor-not-allowed' : ''}`}
              title={isInvitee ? 'Owner controls this' : 'Toggle sharing'}
            >
              <span className={`inline-block h-2.5 w-2.5 rounded-full ${settings?.share_enabled ? 'bg-emerald-400' : 'bg-gray-300'}`} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <div className="space-y-5">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/80">Add a quick title first (e.g., “Japan”). Drag to a column; fill in details when you’re ready.</p>
                  </div>
                </div>
                <div className="flex items-start space-x-3">
                  <Heart className="h-6 w-6 text-white mt-1" />
                  <div>
                    <h3 className="text-lg font-medium text-white">Align with shared meaning</h3>
                    <p className="text-white/80">Capture the “why”—connection, adventure, celebration—to keep motivation high.</p>
                  </div>
                </div>
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/80">Track progress from <b>Not started → Planning → In progress → Booked → Completed</b>.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white/10 rounded-lg p-6">
                ...
                <ul className="text-white/80 space-y-2">
                  ...
                  <li>• Turn on <b>Sharing</b> to collaborate with your partner</li>
      ...
      {/* Board */}
      {loading ? (
        <div className="rounded-xl border border-gray-200 bg-white p-10 text-center text-gray-400">Loading…</div>
      ) : (
            ...
            return (
              ...
              >
                <div className={`bg-gradient-to-r ${meta.headGradient} rounded-xl p-4 mb-4 border-2 border-white`}>
                  ...
                  <p className={`text-xs text-center mt-1 ${meta.countColor}`}>{list.length} item{list.length !== 1 ? 's' : ''}</p>
                </div>
                    ...
                    list.map((it) => (
                      ...
                      >
                        <div className="flex items-start gap-2">
                          <div className="mt-0.5 text-white/60"><Grip className="h-4 w-4" /></div>
                          <button onClick={() => openEditor(it)} className="text-left flex-1 min-w-0" title="Click to edit">
                            <div className="text-[13px] font-medium text-white break-words line-clamp-2">{it.title || 'Untitled'}</div>
                            <div className="mt-1 text-[11px] text-white/85 space-y-1">
                              ...
                              <div className="flex flex-wrap items-center gap-1">
                                {it.target_date && (<span className="px-1.5 py-0.5 rounded bg-black/25 whitespace-nowrap">{fmtDate(it.target_date)}</span>)}
                                {typeof it.priority === 'number' && (<span className="px-1.5 py-0.5 rounded bg-black/25 whitespace-nowrap">Priority {it.priority}</span>)}
                          ...
                          {it.status !== 'completed' && (
                            <button onClick={quickComplete(it)} className="ml-1 shrink-0 p-1 rounded hover:bg-black/20 text-emerald-200" title="Mark completed">
                  ...
                  {GRADIENT_OPTIONS.map((grad, idx) => (
                    <button
                      ...
                      className={`h-12 rounded-lg bg-gradient-to-br ${grad} transition-all ${
                        editor.color_index === idx ? 'ring-4 ring-white scale-105' : 'ring-2 ring-white/30 hover:ring-white/60'
                      }`}
                      title={`Color ${idx + 1}`}
                    />
                  ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditor((x) => ({ ...x, budget_estimate: e.target.value === '' ? null : Number(e.target.value) }))}
                    className="w-full rounded-lg border border-gray-200 bg-white text-gray-900 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#01B1AF]/40"
