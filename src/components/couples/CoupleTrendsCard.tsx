...
function buildMergedActivityGroups(types: CoupleActivityType[]): MergedActivityGroup[] {
  const map = new Map<string, MergedActivityGroup>();
  for (const t of types) {
    const key = normalizeName(t.name);
    const existing = map.get(key);
    if (!existing) {
      map.set(key, { key, name: t.name, color: t.color, ids: [t.id] });
    } else {
...
export default function CoupleTrendsCard({ className = "", title = "Trends" }: Props) {
  ...
  const [mergedGroups, setMergedGroups] = useState<MergedActivityGroup[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const t = await listTypes(ownerId);
        setTypes(t);
        setMergedGroups(buildMergedActivityGroups(t));
      } finally {
        setLoading(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const l = await listLogs(ownerId, startISO, endISO);
        setLogs(l);
      } finally {
        setLoading(false);
      }
  ...
  const { chartData, seriesDefs } = useMemo(() => {
    ...
    if (rangeKey === "week") {
      ...
      for (let q = 0; q < 4; q++) {
        ...
        if (qStart <= range.end) {
          periods.push({
            label: `Q${q + 1}`,
            start: qStart,
            end: qEnd > range.end ? range.end : qEnd,
            isFuture: qStart > today,
          });
        }
      }
    } else {
      ...
      while (y <= range.end) {
        ...
        periods.push({
          label: format(y, "yyyy"),
          start: y,
          end: yEnd > range.end ? range.end : yEnd,
          isFuture: y > today,
        });
    ...
    activeGroups.forEach((g) =>
      seriesMap.set(g.key, { name: g.name, color: g.color, values: periodsNow.map(() => 0) })
    );

    // Aggregate across merged groups
    logs.forEach((log) => {
      const group = mergedGroups.find((g) => g.ids.includes(log.activity_type_id));
      if (!group) return;

      const idx = periodsNow.findIndex(
        (p) =>
          new Date(log.activity_date + "T00:00:00") >= p.start &&
          new Date(log.activity_date + "T00:00:00") <= p.end
      );
      if (idx >= 0) {
        const s = seriesMap.get(group.key);
        if (s) s.values[idx] += log.count ?? 1;
      }
    });
    ...
    const defs = Array.from(seriesMap.values()).map((s) => ({
      key: s.name,
      color: s.color,
    }));

    // 2) trim trailing all-zero buckets (prevents dip at the end of current month/quarter)
  ...
  return (
    <div className={`bg-white rounded-lg border-2 border-gray-200 overflow-hidden shadow-md ${className}`}>
      <div className="bg-gradient-to-br from-[#e88584] to-[#8e4f63] p-4">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex gap-2">
            ...
            <div className="flex items-center gap-1 ml-2">
              <button
                onClick={goPrev}
                className="px-2 py-1 text-xs rounded bg-white border border-gray-200 hover:bg-gray-50 text-[#e88584] hover:text-[#8e4f63] font-bold"
            ...
            {mergedGroups.map((g) => {
              ...
              return (
                <button
                  ...
                  style={{
                    backgroundColor: active ? g.color : "#fff",
                    borderColor: active ? "transparent" : g.color,
                  }}
                >
                  <span className="inline-flex items-center gap-2">
                    <span
                      ...
                      style={{
                        backgroundColor: active ? "rgba(255,255,255,0.85)" : g.color,
                      }}
                    />
                    {g.name}
                    {g.ids.length > 1 && (
                      <span className="text-[10px] opacity-75">({g.ids.length})</span>
                    )}
                  </span>
                </button>
              );
              ...
              <div className="h-full flex items-center justify-center">
                <div className="text-center text-gray-500">
                  ...
                  <div className="text-xs text-gray-400 mt-2">
                    Add some activities to see trends here.
                  </div>
                </div>
              </div>
            ) : (
