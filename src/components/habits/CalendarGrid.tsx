...
/** A single dot/marker rendered under the day number */
export type MarkItem = { id: string; color: string; label: string };

type Props = {
  currentDate: Date;
  view: HabitViewRange;               // <-- use the type from types/habits
  marks: (dateISO: string) => MarkItem[];
  onCellClick: (dateISO: string) => void;
};

export default function CalendarGrid({ currentDate, view, marks, onCellClick }: Props) {
  const buildSections = () => {
    if (view === 'week') {
  ...
  const DayCell = ({ d }: { d: Date }) => {
    ...
    return (
      <button
        ...
        `}
        aria-label={`${format(d, 'MMM d, yyyy')} — ${items.length} habits`}
      >
